...
def save_model(model, epoch, save_path="checkpoint", file_name="jepa_"):
...
def train(model, train_loader, num_epochs=50, learning_rate=1e-4, device=None):
    ...
    for epoch in tqdm(range(num_epochs), desc='Epochs'):
        ...
        for states, actions in train_loader:
            ...
            predicted_states, target_states, encoded_wall = model(states, actions)
            
            mse_loss, std_loss, cov_loss, env_std_loss = model.loss(predicted_states, target_states, encoded_wall)
